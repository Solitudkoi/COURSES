...
module CtrlUnit(
    ...
    assign ImmSel = {3{I_valid | JALR | L_valid | CSR_valid}} & Imm_type_I |
    ...
    assign ALUSrc_B = I_valid | L_valid | S_valid | LUI | AUIPC | CSR_valid;
    ...
    assign ALUControl = {4{ADD | ADDI | L_valid | S_valid | AUIPC | CSR_valid}} & ALU_ADD  |
                        {4{SUB}}                                                  & ALU_SUB  |
                        {4{AND | ANDI}}                                           & ALU_AND  |
                        {4{OR | ORI}}                                             & ALU_OR   |
                        {4{XOR | XORI}}                                           & ALU_XOR  |
    ...
    assign rs1use = R_valid | I_valid | L_valid | S_valid | B_valid | JALR |
                    CSRRW | CSRRS | CSRRC;
    ...
    assign hazard_optype = {2{L_valid}} & hazard_optype_LOAD |
                           {2{S_valid}} & hazard_optype_STORE |
                           {2{R_valid | I_valid | JAL | JALR | LUI | AUIPC}} & hazard_optype_ALU |
                           {2{~(L_valid | S_valid | R_valid | I_valid | JAL | JALR | LUI | AUIPC)}} & hazard_optype_NOP;
